...
public class MovieUtility {
    ...
    public static void main(String[] args) {
        ...
        Integer i1 = Integer.valueOf(12); // new Keyword is inside the method of valueOf
        Movie result = getJokerMovieObject();
        System.out.println("Name is "+ result.getName());// to get only name
    ...
    public static void printMovieNameCharacters(Movie movieObj) {
        //TODO YOUR CODE HERE
        for (int i = 0; i < movieObj.getName().length()-1; i++) {
            System.out.print(movieObj.getName().toUpperCase().charAt(i) + "-");
        }
        // remove the last "-"
        //print each char and "-" one by one till last char, then concatenate last char only. (Without "-")
        //this is easier and shorter for compiler, it does not has to check if statement each and every time!

        System.out.println(movieObj.getName().toUpperCase().charAt(movieObj.getName().length()-1));

  //      2nd way for not adding "-" to the very end of the word
//            for (int i = 0; i < movieObj.getName().length(); i++) {
//                System.out.println(""+movieObj.getName().toUpperCase().charAt(i));
//                // until current char is not the last char; keep concatenating "-".
...
//            }
    ...
    //create a static method that return move object with name Jokjer 2.5 hour and drama
