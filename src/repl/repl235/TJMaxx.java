...
public class TJMaxx {
    ...
    public List<String> getAllItemNames() {
        ...
        for (Item i : regularItems) {
            allTogether.add(i.getName());

        }
        for (OnSaleItem j : onSaleItems) {
            allTogether.add(j.getName());
        }
    ...
    public double getItemPrice(int catalogNumber)
    {
        for ( Item each: regularItems) {
            ...
            for ( OnSaleItem saleEach : onSaleItems) {
                if ( saleEach.getCatalogNumber()==catalogNumber)
                    return  saleEach.getPrice();
    ...
    for (int x = 0; x < regularItems.size() ; x ++ ) {
        if (regularItems.get(x).getCatalogNumber() == catalogNumber) {
            if (regularItems.get(x).getQuantity() > 1) {
                regularItems.get(x).setQuantity(regularItems.get(x).getQuantity() - 1);
            } else {
        ...
        for (int x = 0; x < onSaleItems.size() ; x ++ ) {
            if (onSaleItems.get(x).getCatalogNumber() == catalogNumber) {
                if (onSaleItems.get(x).getQuantity() > 1) {
                    onSaleItems.get(x).setQuantity(onSaleItems.get(x).getQuantity() - 1);
                } else {
                    removeItem(catalogNumber);
                    // break ;
                }
