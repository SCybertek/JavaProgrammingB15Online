...
public class Book {
    ...
    private int id;
    private int pages; //numbe rof pages on the same object (subject )
    ...
    public static Boolean isTitleWord(String word) {
        ...
        //lets add a conditional saying if word contains articles
        //but should we check every single preposition?
        //  List<String> groceries = Arrays.asList("Eggs", "Milk", "Butter", "Apple", "Salmon") ;
        //        // thi sis undmodieable list .. go get your grocery

        String[] allArticles = {"a", "an", "the", "and", "but", "for", "nor", "or", "so", "yet", "at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};

        for (String each : allArticles) {
    ...
    public void setAuthor(String author) {
        if (author.isEmpty()) {
            ...
            String [] arg = author.split(" "); // split by space
            String output = ""; // string to save the new authors name
            for (String each: arg ){
                output+=capitalize(each) + " "; // loop through each word and capitalize each
            }
           setAuthor(output);
    ...
    public void setTitle(String title) {
        ...
        String [] arr = title.split(" "); //split title by space == we get total word number = arr.length
        if (title.isEmpty()){
            this.title = "";
        } else if (arr.length == 1) {
            capitalize(arr[0]); //if title is single word, capitalize it and assign to this.title.
        } else  {

           String t = capitalize(arr[0]);
